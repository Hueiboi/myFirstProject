...
async function loadCart() {
    ...
    const res = await fetch(`${URL.BASE_URL}/cart?user_id=${user_id}`);
...
async function updateCartItems(product_id, oldQuantity) {
    ...
    const res = await fetch(`${URL.BASE_URL}/cart/${product_id}`, {//Sửa lại fetch URL có đính kèm user_id
        method: 'PUT',
...
async function deleteCartItems(product_id) {
    ...
    if(confirm("Are you sure to delete this item ?")) {
        const res = await fetch(`${URL.BASE_URL}/cart/${product_id}`, {
        method: 'DELETE',
        headers: {'Content-type': 'application/json'},
