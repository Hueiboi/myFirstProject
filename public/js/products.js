const URL = require('./url'); // Đổi nếu backend bạn đang dùng port khác
...
async function loadProducts() { // Tải danh sách sản phẩm từ API
  ...
  const res = await fetch(`${URL.BASE_URL}/products?sortBy=${sortBy}&order=${sortOrder}`); // Gọi API với tham số sắp xếp
...
async function searchProducts() {
  ...
  const res = await fetch(`${URL.BASE_URL}/products/name?filter=${filter}&sortBy=${sortBy}&order=${sortOrder}`); // Gọi API với từ khóa tìm kiếm
  const resData = await res.json();
  renderProductList(resData.data);
...
async function createProduct() { // Tạo sản phẩm mới
  ...
  const res = await fetch(`${URL.BASE_URL}/products`, { // Gửi dữ liệu lên server
...
async function deleteProduct(id) {
  if(confirm("Are you sure?")){ // Xác nhận trước khi xóa
    await fetch(`${URL.BASE_URL}/products/${id}`, {method: 'DELETE'});
  }
  loadProducts();
}
...
async function updateProduct(id, name, price, stock_quantity) { // Cập nhật sản phẩm
  ...
  if (newName && newPrice && newQuantity) {
    await fetch(`${URL.BASE_URL}/products/${id}`, {
      method: 'PUT',
